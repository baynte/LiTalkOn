...
import { fetchVoiceClips, createExamTest } from '../../services/api';
import { VoiceClip } from '../../types';
import Icon from 'react-native-vector-icons/MaterialCommunityIcons';

interface CreateExamTestScreenProps {
  navigation: any;
}

const CreateExamTestScreen: React.FC<CreateExamTestScreenProps> = ({ navigation }) => {
  ...
  // Handle create exam test
  const handleCreateExamTest = async () => {
    // Validate inputs
    if (!name.trim()) {
    ...
    try {
      ...
      Alert.alert(
        'Success',
        'Exam test created successfully',
        [
          {
            text: 'OK',
      ...
      console.error('Error creating exam test:', err);
      setError(err.message || 'Failed to create exam test');
      Alert.alert('Error', err.message || 'Failed to create exam test');
    } finally {
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.text }]}>Create Exam Test</Text>
      </View>
...
export default CreateExamTestScreen; 